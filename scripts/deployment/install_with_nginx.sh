...
# Complete Installation script for Swath Movers with Nginx CDN
# Run this script with: bash install_with_nginx.sh

set -e

echo "========================================"
echo "Swath Movers Complete Installation"
...
echo ""
sudo systemctl status nginx.service --no-pager -l | head -15
echo ""
...
echo ""
echo "# Test static file caching:"
echo "  curl -I http://localhost/static/css/table.css"
echo ""
echo "# Check Nginx cache status:"
echo "  curl -I http://localhost/get_coordinates (look for X-Cache-Status header)"
echo ""
